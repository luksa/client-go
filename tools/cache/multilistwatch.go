...
// ListerWatcher generator func and returns a single ListerWatcher
// capable of operating on multiple namespaces.
func MultiNamespaceListerWatcher(namespaces []string, f func(string) ListerWatcher) ListerWatcher {
	// If there is only one namespace then there is no need to create a proxy.
	if len(namespaces) == 1 {
		return f(namespaces[0])
	}
	var lws []ListerWatcher
...
// multiListerWatcher abstracts several ListerWatchers, allowing them
// to be treated as a single ListerWatcher.
type multiListerWatcher []ListerWatcher
...
// watch.Interface of every ListerWatcher into a single result chan.
func (mlw multiListerWatcher) Watch(options metav1.ListOptions) (watch.Interface, error) {
	...
	if options.ResourceVersion != "" {
		...
		if len(rvs) != len(mlw) {
			debug.PrintStack()
			return nil, fmt.Errorf("expected resource version to have %d parts to match the number of ListerWatchers, but was: %s", len(mlw), options.ResourceVersion)
		}
...
// Watch funcs errored. The length of []ListerWatcher and []string must
// match.
func newMultiWatch(lws []ListerWatcher, resourceVersions []string, options metav1.ListOptions) (*multiWatch, error) {
	var (
